...
\begin{itemize}
	...
	\item Teorema di Noether (corrispondenza tra simmetrie e quantità conservate): invarianza temporale $\implies$ conservazione dell'energia meccanica; invarianza spaziale $\implies$ conservazione della quantità di moto; invarianza rotazionale $\implies$ conservazione del momento angolare. \index{Teorema di Noether}
\end{itemize}
...
Fino a qui si è in sostanza parlato di dinamica delle particelle. Ora è necessario introdurre la dinamica delle onde, in quanto la \MQ si basa pesantemente sulle proprietà delle onde.
...
\subsubsection*{Restrizioni e Libertà di \texorpdfstring{$(V, \vect A)$}{}}
Risulta che $V$ e $\vec A$ possono essere quantificati a posteriori a partire dalle misurazioni di \evect E ed \evect B. Risulta però che non sono univocamente determinate:
\[
	\begin{cases}
		V' = V + \beta \\
...
Data una funzione $f$ integrabile da $\R^n$ in $\R^n$, esiste una funzione $\tilde f$ tale che
\[
...
La varianza di un operatore o di una funzione $A$ è indicata con $\var A$ ed è definita nel seguente modo:
\[
	\var A^2 =\ \avg{A^2} - \avg{A}^2
\]
dove $\avg f$ è la media sullo stato, che, dato uno stato (una funzione $\L^2$) generico $\psi(x)$, equivale all'integrale
\[
