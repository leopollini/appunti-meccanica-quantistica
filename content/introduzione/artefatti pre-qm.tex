...
\begin{tikzpicture}[scale=0.8]
	...
	\draw[thick, blue] plot[domain=0.01:5, samples=30] (\x, {\x^3/(exp(\x)-1)});
    \draw[domain=0:2,smooth,variable=\x,red,thick,dashed] plot (\x,{(\x)*(\x)});
	\node at (4,4) {Rayleigh-Jeans}; 
	\node at (5,1.5) {Planck};
\end{tikzpicture}
...
Max Planck risolse questo "paradosso" introducendo una ipotesi rivoluzionaria: l'energia può essere emessa o assorbita solo in quantità discrete (quanti) proporzionali alla frequenza; è come se gli emettitori di energia elettromagnetica fossero finiti, anche se moltissimi:
\[
	E = h\nu
\]
dove $h$ è la costante di Planck. A partire da questo è possibile arrivare alla legge di Planck per la distribuzione densità di energia luminosa in per frequenza:
...
\begin{tikzpicture}[scale=0.8]
	\draw[->] (0,0) -- (6,0) node[right] {$\nu$};
	\draw[->] (0,0) -- (0,4) node[above] {$u(\nu,T)$};
	\draw[thick, red] plot[domain=0.01:5, samples=20] (\x, {\x^3/(exp(\x/1.3)-1)});
	\draw[thick, blue] plot[domain=0.01:5, samples=20] (\x, {\x^3/(exp(\x/1.1)-1)});
	\draw[thick, green] plot[domain=0.01:5, samples=20] (\x, {\x^3/(exp(\x)-1)});
	\node at (3.5,3.5) {$T_1 > T_2 > T_3$};
	\node[red] at (5.2,3) {$T_1$};
	\node[blue] at (5.2,1.6) {$T_2$};
	\node[green] at (5.2,1.05) {$T_3$};
\end{tikzpicture}
...
Da evidenziare il fatto che l'unità di misura di $\h$ è $Js$, Joule per secondo.