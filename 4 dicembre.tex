...
Nella base delle energie l'equazione di Schroedinger diventa:
\[
...
\]
ed $E = E_{cm} + E_{rel}$. Siccome entrambe queste funzioni rispettano l'equazione di Schroedinger:
\[
...
Vabè ora abbiamo le armoniche sferiche $Y_l^m(\theta, \phi) = C_{lm}e^{im\phi}P_l^m(\cos \theta)$
con \(P_l^m(cos \theta) = C_l^{\abs m}(-1)^m(\sin \theta)^{\abs m} \frac{\partial^{\abs m}}{\partial \xi^{\abs m}}P_l(\cos \theta)\)

Ora che sappiamo la componente angolare per OGNI problema a potenziale radiale, possiamo tornare alla componente radiale dello stato:
\[
