...
\end{align*}
È figo anche l'operatore numero $\N: \N\state n = n\state n $
...
Altre coses:
\[
    \bra x | \hat a | 0 \ket = 0 \implies x \bra x | o \ket + \frac{i}{m\omega} \bra x | \hat p | 0 \ket = 0
\]
...
\begin{tikzpicture}[scale=1]
    ...
    \node[anchor=west,red,font=\small] at (2.3,{exp(-2.3*2.3)+1}) {$y=e^{-x^2}+1$};
    % horizontal asymptote for the gaussian
    \draw[dashed,gray] (-3.2,1) -- (3.2,1) node[anchor=west,gray,font=\small] {$y=1$};
    \draw (1,1) -- (1,0) node[left,font=\small] {};
    \draw (-1,1) -- (-1,0) node[left,font=\small] {};
\end{tikzpicture}

Quel che risulta è che siccome una particella quantistica è delocalizzata, risulta che essa ha un comportamento simile a una particella classica con abbastanza energia da potersi muovere nella zona di delocalizzazione, ovvero una particella che ha energia $E_0$.
...
\begin{tikzpicture}[scale=1]
    ...
    \node[anchor=west,green!60!black,font=\small] at (2.3,{2.3*exp(-2.3*2.3)+2.5}) {$y=x e^{-x^2}+\frac{5}{2}$};
    % horizontal asymptote for the gaussian
...
Inoltre, in generale, per una coppia di stati elementari $|n\ket$ e $|k\ket$:
\begin{align*}
    \bra n | \hat X | k \ket = \\
    \bra n | \hat P | k \ket = 
\end{align*}
...
\subsection*{Dinamica}
Risulta, passando dalla rappresentazione di Schroedinger a quella di Heisenberg:
\[
...
Prossime cose: \textbf{Teoria generale del momento angolare}, \textbf{Perturbation Theory}