...
\begin{align*}
    ...
    &= \bra \psi, t_0 | e^{\frac{i}{\hbar} \H (t - t_0)} \hat A e^{-\frac{i}{\hbar} \H (t - t_0)} | \psi, t_0 \ket
\end{align*}
...
\]
(si può dimostrare per induzione come esercizio). Questa operatzione è molto simile alla derivata, infatti risulta:
\[
    \left[ \hat x, f(\hat p) \right] = i \hbar \deriv[\hat p]{} f(\hat p)
\]
E lo stesso vale scambiando $\hat p$ con $\hat x$. E quindi
\begin{align*}
    \left[\hat x, f(\hat p)\right] &= i\hbar \deriv[\hat p]{} f(\hat p) \\
    \left[\hat p, f(\hat x)\right] &= -i\hbar \deriv[\hat x]{} f(\hat x)
\end{align*}