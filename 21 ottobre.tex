...
\begin{align*}
    |\vec x\ket = |x,y,z\ket \\
    \hat{X} |\vec x\ket = x |\vec x\ket \\
    \hat{Y} |\vec x\ket = y |\vec x\ket \\
    \hat{Z} |\vec x\ket = z |\vec x\ket \\
    \vec{\hat{X}} |\vec x\ket = \vec x |\vec x\ket
\end{align*}
...
\end{gather*}
Che coincide con la definizione della trasformata di Fourier! E anche: siccome invertire i termini del prodotto scalare coincide col fare il complesso coniugato risulta: ???
