...
\[
    |\psi\ket = \int \ \ketbra{\xi}{\psi} \Ket \xi d\xi = \int \ \bra \xi | \psi \ket |\xi \ket d\xi
\]
dove $\ketbra{\xi}{\psi}$ è il proiettore con variazione continua.
Ovviamente si pone:
\[
    \bra \xi | \xi' \ket = \delta(\xi - \xi')
\]
Inoltre un autovettore in uno spazio di questo tipo ha questo aspetto:
\[
    \hat X |\xi\ket = x |\xi\ket
\]
E per finire l'espansione dell'identità diventa:
\[
...
\begin{align*}
    ...
    \hat{Y} \vpos = y \vpos \\
    \hat{Z} \vpos = z \vpos \\
    \vec{\hat{X}} \vpos = \vec x \vpos
\end{align*}
Poi è tipico scriver $\Ket {\vec x}$ al posto di $\vpos$ per comodità di scrittura.

\subsubsection*{Altro esempio: operatore di traslazione}
Chiamiamo $\tau$ l'operatore che si comporta in questo modo:
\[
    \tau (\vec {dx}) \vpos = \Ket {\vec x + \vec{dx}}
\]
Al chè si può usare il solito trucco:
\[
    \tau (\vec {dx}) \hat{Id} |\psi\ket = \rnint |\vec x + \vec {dx}\ket \bra \vec x | \psi\ket d^nx
\]
...
\begin{align*}
    [\hat{X}_j, \tau(\vec{dx})] \vpos &= \hat{X}_j \tau(\vec{dx}) \vpos - \tau(\vec{dx}) \hat{X}_j \vpos \\
    &= \hat{X}_j |\vec x + \vec{dx}\ket - \tau(\vec{dx}) \vec x \vpos \\
...
\begin{gather*}
    ...
    = \rnint \ketbrabra {\vec x} \psi\ket  - |\vec x \ket \bra \vec x |  \vec{dx} \cdot \hat{\Grad} |\psi\ket d^nx = \\
    = \left[\hat {Id} - \rnint \ketbrabra{\vec x} \vec{dx} \cdot \hat \hat{\Grad} d^nx \right] \Ket \psi
\end{gather*}
Ma ricordando la definizione di $\tau$ risulta che:
\begin{gather*}
    \tau(\vec{dx})\Ket \psi = \l \hat {Id} - i\frac{\hat p}{\h} \cdot \vec{dx}\r\Ket \psi = \left[\hat {Id} - \rnint \ketbrabra{\vec x} \vec{dx} \cdot \hat \Grad d^nx \right] \Ket \psi = \Ket \psi - \rnint \vec{dx} \cdot \hat \Grad \psi(x) d^nx \Ket {\vec x} \\
    \Bra {\vec x} \hat p \Ket \psi = \Bra {\vec x} \l -i\h\hat\Grad \r \Ket\psi = -i\h\Grad \braket{\vec x}{\psi}
\end{gather*}
Confrontando la prima e l'ultima espressione si equazione esprime l'azione dell'operatore momento ($\hat p$) quando lo stato è espresso nella base delle posizioni: in particolare ne è il gradiente!
...
\begin{gather*}
    ...
    = \frac{1}{(2 \pi \hslash)^{n/2}} \rnint e^{\frac{i}{\hslash} \vec p \cdot \vec x} \tilde\psi(\vec p) d^np
\end{gather*}
Che coincide con la definizione della trasformata di Fourier!

Se poi si procede allo stesso modo scambiando $\hat p$ e $\hat x$ (dimostrando che vale lo stesso l'equivalenza del gradiente) si ottiene anche:
\begin{gather*}
    \bra \vec p | \psi \ket = \tilde\psi(\vec p) = \\
...
Il risultato interessante è il fatto che gli operatori $\hat p$ e $\hat x$ non sono compatibili: dato che il passaggio da uno all'altro si fa tramite trasformata di Fourier, il prodotto delle loro varianze deve essere $\sim 1$, ovvero vi è indeterminazione! Attenzione però: questo fatto è solo una conseguenza delle definizioni utilizzate, non un teorema!