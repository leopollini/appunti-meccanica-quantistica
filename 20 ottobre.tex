...
Siccome si tratta solo di un formalismo non è data una definizione formale. Semplicemente, approfittando del fatto che gli stati formano uno spazio vettoriale si stabilisce un modo per indicare i "vettori" di questo spazio, che saranno gli stati:
\[
    |\psi \ket
\]
...
\[
    \bra \psi |
\]
Siccome è sempre possibile per uno spazio vettoriale generare una base, allora si indica con $|\psi_j\ket$ il vettore generico della base, con $j \in J$ insieme generico, non necessariamente numerabile.
...
\]
Al chè si ha la rappresentazione, per esempio, dell'operatore di Stern e Gerlak $\SG z$:
\[
...
\subsubsection*{Esempio: esperimento di Stern e Gerlak}
...
Dall'esperimento di Stern e Gerlak fatto usando $\hat S_z$ e $\hat S_x$ si sa che se si misura $S_x$ su uno stato $|+_z\ket$ si ottiene con probabilità 1/2 sia $|+_x\ket$ che $|-_x\ket$. Quindi si può scrivere:
\begin{align*}
